...
class _ListScreenState extends State<ListScreen> {
  ...
  _scrollCall(){
    setState(() {
      _future = _db.getPaginateRecords(fetchRecords);
    });
  }
  @override
  void initState() {
          ...
          builder: (BuildContext ctx, AsyncSnapshot shot){
            if(!shot.hasData){
              ...
              return ListView.builder(
                  ...
                  itemBuilder: (c, index){
                    return ListTile(
                      title: Text(shot.data[index]['name'],style: TextStyle(fontSize: 20),),
                      subtitle: Text(shot.data[index]['salary'].toString(),style: TextStyle(fontSize: 20)),
                    );
                  });
            }
